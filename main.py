...
def callback():
    ...
    anaplan = OAuth2Session(client_id, redirect_uri=redirect_uri,
                           state=session['oauth_state'])
    token = anaplan.fetch_token(token_url, client_secret=client_secret,
...
def menu():
    ...
    """ % pformat(session['oauth_token']['access_token'], indent=4)
...
def automatic_refresh():
    ...
    anaplan = OAuth2Session(client_id,
                           token=token,
