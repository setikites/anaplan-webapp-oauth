...
anaplan = OAuth2Session(client_id, scope=scope, redirect_uri=redirect_uri)  # failed with client=anaplan_client

# Redirect user to Anaplan for authorization
authorization_url, state = anaplan.authorization_url(authorization_base_url,
...
token = anaplan.fetch_token(token_url, client_secret=client_secret, authorization_response=redirect_response)
print ('\n=== TOKEN ===')
...
headers = {
    'Authorization': f'AnaplanAuthToken {token["access_token"]}'
}
